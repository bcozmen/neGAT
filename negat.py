...
class Agent():
    def __init__(self, num_agents, node_dim, device):
...
#node dim
# 8 + [gworth_threshold, pruning_threshold, surface markers ]

class PhysicsEngine(nn.Module):
    def __init__(self, num_agents=1000, num_heads=4, node_dim=8 + 4, 
    ...
    def plot_graph(self):
        va = self.V_agent.cpu().numpy()
        inp_mask = (va[:, 1] == 0) & (va[:, 0] > 0)
        x = self.V[inp_mask][:, 0].cpu().numpy()
        y = self.V[inp_mask][:, 1].cpu().numpy()
        energy = self.V[inp_mask][:, 2].cpu().numpy()
        
        theta = np.linspace(0, 2*np.pi, 100)   
        plt.plot(0.5 + 0.30 * np.cos(theta), 0.5 + 0.30 * np.sin(theta), 'r--', alpha=0.5, label='Ring Boundaries')
        plt.plot(0.5 + 0.45 * np.cos(theta), 0.5 + 0.45 * np.sin(theta), 'r--', alpha=0.5, label='Ring Boundaries')

        current_phase = np.sin(2 * np.pi * self.step / self.phase_steps)
        ...
        plt.title(f'Agent Positions at Step {self.step} (Phase Val: {current_phase:.3f})')
        ...
        avg_energy = np.array(self.avg_energy)
        ...
        plt.plot(self.deaths_over_time, label='Deaths per Step')
    ...
    def train(self, steps=1000):
        for _ in trange(steps, desc="Training Simulation"):
            self.forward()
    def forward(self):
        ...
        deaths = self.delete()      
        self.repopulate()  
        self.deaths_over_time.append(deaths)
        energies = self.V[inp_mask][:, 2].cpu().numpy()
        self.avg_energy.append(np.percentile(energies, [0, 25, 50, 75, 100]))
        return deaths
    ...
    def _apply_physics(self, V):
        ...
        V[inp_indices, 0] = V[inp_indices, 0] + V[global_out_indices, 0]
        V[inp_indices, 1] = V[inp_indices, 1] + V[global_out_indices, 1]  
        ...
        # ---------------------------------------------------------
        # 1. Stack coordinates to get an (N, 2) tensor of positions
        positions = torch.stack([x_coord, y_coord], dim=1)
        
        # 2. Calculate pairwise distance matrix (N, N)
        dist_matrix = torch.cdist(positions, positions)
        
        # 3. Count neighbors within a certain radius (e.g., 0.05)
        # We subtract 1.0 so an agent doesn't count itself
        density_radius = 0.025
        density_count = (dist_matrix < density_radius).float().sum(dim=1) - 1.0
        density_count = torch.clamp(density_count, min=0.0)
        
        # 4. Feed density to the agent as an input feature for the next step.
        # We store it at index 3. We divide by the number of agents to keep the NN input roughly scaled.
        V[inp_indices, 3] = density_count / self.num_agents
        
        # Calculate penalty to apply to energy
        density_penalty = (density_count / self.num_agents) * 0.2
        # ---------------------------------------------------------
        ...
        x_mask = x_mask.float() * 2.0 - 1.0
        
        # Dynamic Smooth Target Y Transition
        phase_signal = np.sin(2 * np.pi * self.step / self.phase_steps)
        target_y = 0.5 + (phase_signal * 0.25)
        y_distance_error = torch.abs(y_coord - target_y)
                
        freq_reward = 0.2 - (y_distance_error * 2.0)
        freq_reward = torch.clamp(freq_reward, -0.20, 0.20)

        ring_rewerd = torch.where(x_mask > 0, 0.05, -0.15)
        
        # Combine ecology constraints PLUS the new density penalty
        energy_change = freq_reward + ring_rewerd - 0.1 - density_penalty 
    ...
    def adapt_network_topology(self):
        pass
